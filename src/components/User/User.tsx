...
const User = () => {
    ...
    return (
        <div>
            {user.auth ? <div className={'flex gap-4 items-center'}>
                    <UserIcon/><Button variant={'outlined'} onClick={logout}>Выйти</Button></div>
                : <Button variant={'filled'} onClick={() => setModal(true)}>Войти</Button>}
            {modal && <Modal>
                {alert && <div>{alert}</div>}
                <Form onSubmit={login} onClose={() => setModal(false)}/>
            </Modal>}
        </div>
    );
};
