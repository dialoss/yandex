...
const ArrowButton = ' shadow-lg bg-white rounded-full p-2 hover:cursor-pointer'

const CardList = ({cards, pages, getPage}: ICardList) => {
    ...
    return (
        <>
            ...
            <div className={'flex gap-2 items-center mt-4'}>
                <div className={(page === 1 && "bg-gray-200") + ArrowButton} onClick={() => page > 0 && setPage(page - 1)}>
                    <ArrowLeft/>
                </div>
                <div>{page}</div>
                <div className={(page === pages && "bg-gray-200") + ArrowButton} onClick={() => page < pages && setPage(page + 1)}>
                    <ArrowRight/>
                </div>
            </div>
        </>
